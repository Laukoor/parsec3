...
class CondException: public std::exception {
  public:
    virtual const char *what() const noexcept /*throw()*/ {return "Unspecified condition error";}
};

//Condition initialization error
class CondInitException: public CondException {
  public:
    virtual const char *what() const noexcept /*throw()*/ {return "Unspecified error while initializing condition";}
};

//Condition destruction error
class CondDestroyException: public CondException {
  public:
    virtual const char *what() const noexcept /*throw()*/ {return "Unspecified error while destroying condition";}
};
